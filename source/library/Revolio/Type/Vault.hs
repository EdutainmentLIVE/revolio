...
type Vault = Stm.TVar (Map.Map Type.SlackUserId Type.StratusTimeCredentials)

makeVault :: IO Vault
makeVault = Stm.newTVarIO Map.empty

insertVault
  :: Vault -> Type.SlackUserId -> Type.StratusTimeCredentials -> IO ()
insertVault vault key = Stm.atomically . Stm.modifyTVar vault . Map.insert key

lookupVault
  :: Vault
  -> Type.SlackUserId
  -> IO (Either String Type.StratusTimeCredentials)
